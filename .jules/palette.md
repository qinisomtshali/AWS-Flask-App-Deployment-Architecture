...
## 2025-05-28 - Enhancing Documentation Navigability
**Learning:** In documentation-heavy repositories where complex diagrams are central, providing a direct "Jump to detailed descriptions" link within the `<figcaption>` significantly improves the user experience. It creates a clear navigational path for users to transition from visual inspection to detailed technical specifications, ensuring that the descriptive text is easily discoverable.
**Action:** Add a contextual jump link (e.g., `(<a href="#target">Jump to detailed descriptions</a>)`) to the `<figcaption>` of primary architectural diagrams that target their respective descriptive sections.
